...
class QueryService:
    ...
    async def get_query_engine(
        ...
        try:
            ...
            if not self.chroma_manager:
                 self.logger.error("ChromaManager is not initialized.")
                 return None

            chroma_client = self.chroma_manager.get_client()
            
            if not chroma_client:
                 self.logger.error("Failed to get ChromaDB client.")
                 return None

            try:
                chroma_collection = chroma_client.get_collection(collection_name)
            except Exception as e:
                 # Check for "does not exist" error variants if needed, or just log
                 self.logger.warning(f"Collection '{collection_name}' not found or error accessing it: {e}")
                 return None
            ...
            try:
                ...
                self.logger.warning(f"Experiment service error: {exp_error}")
            ...
            self.logger.error(f"Failed to create query engine for collection '{collection_name}': {e}", exc_info=True)
            return None
    ...
    async def query_with_context(
        ...
        try:
            # Step 0: Check Cache (Phase I.1)
            cache_params = {
                "collections": collection_names,
                "n_results": n_results,
                "model": llm_model or self.config.get("LLM_MODEL"),
            ...
            try:
                ...
                cached_response = redis_cache.get_query_result(query_text, cache_params)
                if cached_response:
                    self.logger.info(f"Cache hit for query: '{query_text}'")
                    # Add cache metadata
    ...
    def _format_structured_prompt(self, query_text: str, context_chunks: List[Any], system_prompt: str) -> str:
        """
        Formats the prompt for the LLM including context and query.
        """
        context_str = "\n\n".join([f"Source: {node.metadata.get('collection_name', 'unknown')}\nContent: {node.content}" for node in context_chunks])
...
Answer:"""
    ...
    def _translate_llm_error(self, error_msg: str) -> str:
        """
        Translates raw LLM error messages into user-friendly messages.
        """
        if "context_length_exceeded" in error_msg:
            return "The query resulted in too much context. Please try to be more specific."
