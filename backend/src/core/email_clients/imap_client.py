...
class IMAPClient(AbstractEmailClient):
    ...
    async def is_authenticated(self) -> bool:
        ...
        async with self._connection_lock:
            try:
                ...
                if self.imap_port in [143, 3143]:
                    logger.info(f"Using non-SSL IMAP connection for port {self.imap_port}")
                    self.imap_client = aioimaplib.IMAP4(
                        host=self.imap_host,
                        port=self.imap_port
                    )
                else:
                ...
                print(f"DEBUG: Auth failed with error type: {type(e)}, repr: {repr(e)}")  # Enhanced debug
                self._authenticated = False
                self.imap_client = None
                return False
    ...
    async def create_draft(self, to: str, subject: str, body: str, thread_id: Optional[str] = None, in_reply_to: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            # Ensure draft folder exists (especially for new accounts/mocks)
            if draft_folder:
                 await self.imap_client.create(f'"{draft_folder}"')

            # Append to Drafts folder
            # aioimaplib.append signature: append(message, mailbox='INBOX', flags=None, date=None)
            # We must pass args in correct order and date must be timezone aware.
    ...
    async def move_to_label(self, message_id: str, label: str) -> bool:
        ...
        try:
            ...
            # Note: message_id here is expected to be the sequence ID or UID depending on client usage
            # aioimaplib store takes: sequence_set, item_name, item_value
            response = await self.imap_client.store(message_id, '+FLAGS', '(\\Deleted)')
            if response.result != 'OK':
                logger.error(f"Failed to mark original as deleted: {response}")
                return False
