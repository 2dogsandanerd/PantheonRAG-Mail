...
BM25_INDEX_DIR = Path(__file__).parent.parent.parent / "data" / "bm25_indices"
...
class BM25Retriever(BaseRetriever):
    ...
    def _load_bm25_index(self):
        ...
        if not bm25_index_path.exists():
            self.logger.warning(
                f"BM25 index not found for collection {self.collection_name} at {bm25_index_path}"
            )
            return
    ...
    def _retrieve(self, query_bundle: QueryBundle) -> List[NodeWithScore]:
        ...
        if not self._bm25_index or not self._nodes:
            self.logger.warning(
                f"BM25 index not initialized for collection {self.collection_name}. Returning empty results."
            )
            return []
        ...
        ranked_indices = doc_scores.argsort()[-top_n:][
            ::-1
        ...
        if self.verbose:
            self.logger.info(
                f"BM25 retrieved {len(results)} nodes for query: {query_bundle.query_str}"
            )
