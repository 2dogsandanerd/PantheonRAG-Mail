    ...
    def _initialize_embeddings(
        ...
        try:
            if provider.lower() == "ollama":
                ...
                return self._initialize_mock_embeddings(model, config)
            else:
...
# Singleton instance
    def _initialize_mock_embeddings(self, model: str, config: Dict) -> Optional[Any]:
        ...
        Returns:
            MockEmbedding instance
        """
        try:
            from llama_index.core.embeddings import MockEmbedding
            
            # Default to 768 dimensions (common size)
            embed_dim = 768
            embeddings = MockEmbedding(embed_dim=embed_dim)
            logger.info(f"Mock embeddings initialized with dim={embed_dim}")
            return embeddings
        except Exception as e:
            logger.error(f"Failed to initialize Mock embeddings: {e}")
            return None
