"""Multi-provider LLM configuration for PantheonMail.
...
def get_config(use_hot_reload: bool = True) -> LLMConfig:
    ...
    if use_hot_reload:
        try:
            ...
            collection_name = config_dict.get(
                "CHROMA_COLLECTION", "mail_knowledge_base"
            )
            project_collection_name = config_dict.get(
        ...
        try:
            ...
            return WebSocketConfig(
                heartbeat_interval=int(
                    config_dict.get("WEBSOCKET_HEARTBEAT_INTERVAL", 30)
                ),
                heartbeat_timeout=int(
                    config_dict.get("WEBSOCKET_HEARTBEAT_TIMEOUT", 90)
                ),
                auth_required=config_dict.get(
                    "WEBSOCKET_AUTH_REQUIRED", "false"
                ).lower()
                == "true",
                localhost_only=config_dict.get(
...
def get_rag_config(use_hot_reload: bool = True) -> RAGConfig:
    ...
    if use_hot_reload:
        try:
            ...
            return RAGConfig(
                reranker_enabled=config_dict.get("RERANKER_ENABLED", "false").lower()
                == "true",
                reranker_model=config_dict.get(
                    "RERANKER_MODEL", "BAAI/bge-reranker-base"
                ),
                reranker_top_k=int(config_dict.get("RERANKER_TOP_K", "5")),
                use_parent_retriever=config_dict.get(
                    "USE_PARENT_RETRIEVER", "false"
                ).lower()
                == "true",
                use_advanced_pipeline=config_dict.get(
    ...
    return RAGConfig(
        ...
        use_parent_retriever=os.getenv("USE_PARENT_RETRIEVER", "false").lower()
        == "true",
        use_advanced_pipeline=os.getenv("USE_ADVANCED_PIPELINE", "false").lower()
        == "true",
    )


def create_llm_instances(
    config: LLMConfig, use_hot_reload: bool = True
) -> Dict[str, Any]:
    ...
    if config.embedding_provider == "ollama":
        ...
        instances["embeddings"] = OllamaEmbedding(
            model_name=config.embedding_model, base_url=ollama_host
        )
    ...
    elif config.embedding_provider == "mock":
        from llama_index.core.embeddings import MockEmbedding

        instances["embeddings"] = MockEmbedding(embed_dim=768)
