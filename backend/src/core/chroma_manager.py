        ...
        if self._client is None:
            try:
                if self._in_memory:
                    ...
                    self.logger.info(f"DEBUG: LLM_PROVIDER={provider}")
                    if provider == "mock":
                        self.logger.info("Creating new MockChromaClient (Mock Mode).")
                        from src.core.mock_chroma_client import MockChromaClient
                    ...
                    # ChromaDB v0.5+ doesn't require tenant/database for basic usage
