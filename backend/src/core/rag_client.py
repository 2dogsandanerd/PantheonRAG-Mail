...
class RAGClient:
    ...
    def __init__(self, config: dict):
        ...
        # Check config OR environment variable for in_memory flag
        in_memory = config.get("CHROMA_IN_MEMORY", False)
        if not in_memory:
            in_memory_env = os.getenv("CHROMA_IN_MEMORY", "False")
            logger.info(f"DEBUG: CHROMA_IN_MEMORY env var: {in_memory_env}")
            in_memory = in_memory_env.lower() == "true"
            
        logger.info(f"DEBUG: RAGClient initializing ChromaManager with in_memory={in_memory}")
    ...
    async def create_collection(self, name: str, **kwargs) -> RAGResponse:
        ...
        # If not provided directly, try to build from kwargs
        if not embedding_config:
            ...
            # Otherwise leave as None to let CollectionManager use defaults
