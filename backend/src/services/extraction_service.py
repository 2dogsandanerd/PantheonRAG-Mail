...
class ExtractionService:
    ...
    async def extract_document(
        ...
        # -- FIAT STRATEGY: Lean Mode Check --
        is_lean_mode = os.getenv("MAIL_EDITION_LEAN", "false").lower() == "true"
        if is_lean_mode:
            logger.info("🖖 Fiat Strategie: Lean Mode aktiv. Priorisiere Docling (Goethe Lane).")
        ...
        if mime_type in docling_formats:
            ...
            try:
                ...
                if docs:
                    extracted_text = docs[0].text
                    # Get structured artifacts (simulated for now, Docling provides them in the result object)
                    # In a real scenario, we'd extract these from the Docling 'result' object
                    loader_artifacts = {
                        "tables_count": extracted_text.count("| --- |"), # Heuristic
                        "headings_count": extracted_text.count("#"),
                        "lists_count": extracted_text.count("\n- "),
                        "images_count": 0
                    }
                else:
                    extracted_text = ""
                ...
                # In Lean mode, we might want to fallback to traditional loaders
                logger.warning("Lean Mode Fallback: Docling fehlschlagen, versuche Standard-Loader.")
        ...
        if not extracted_text:
            if mime_type and mime_type.startswith("text/"):
                ...
                try:
                    from src.services.loaders import email_loader
                ...
                from src.services.loaders import image_loader
        ...
        final_quality_score = quality_scorer.score_extraction(
            ...
            language=language,
            ocr_confidence=loader_artifacts.get("ocr_confidence", 1.0 if extraction_engine == "docling" else 0.5),
            structure_score=structure_score
        )

        # -- FIAT STRATEGY: Confidence-Trigger (Phase 1) --
        # If quality is too low, we normally would trigger the "Refinery" (Multi-Lane).
        # In this edition, we log it and potentially flag it for Human-In-The-Loop.
        if is_lean_mode and final_quality_score < 0.85:
            logger.warning(f"❗ Confidence-Trigger: Niedrige Qualität ({final_quality_score:.2f}) für {filename}. "
                           "In der 'Refinery' Edition würde hier der Multi-Lane-Consensus starten.")
