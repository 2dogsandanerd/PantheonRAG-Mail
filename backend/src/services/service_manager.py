...
from src.services.health_checker import health_checker, ServiceStatus
...
class ServiceManager:
    ...
    def __init__(self):
        ...
        # Use global health checker (Phase 5)
        self.health_checker = health_checker

    def _invalidate_status_cache(self):
        """Invalidate status cache (delegates to HealthChecker)."""
        self.health_checker.invalidate_cache()
    ...
    async def connect_to_chroma(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """
        Connect to running ChromaDB service.

        Standard port: 8000
        Health check endpoint: /api/v1/heartbeat (ChromaDB 0.5.x uses v1 API)
        """
        # Get configured host or use default
        config_chroma_host = config.get("CHROMA_HOST", "http://chromadb:8000")
        ...
        try:
            ...
            # Check if service is running using v1 API (ChromaDB 0.5.x)
    ...
    async def get_status(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """Get service status (delegates to HealthChecker)."""
        status = await self.health_checker.get_all_status(
            service_configs=self.service_configs,
            config=config
        )

        # Update internal status based on health check
        self.status["ollama"] = status["ollama"]["status"]
        self.status["chroma"] = status["chroma"]["status"]

        # Merge with additional config data if available
        ollama_config = self.service_configs.get("ollama")
        chroma_config = self.service_configs.get("chroma")

        if ollama_config:
            status["ollama"]["host"] = ollama_config.get("host")
            status["ollama"]["port"] = ollama_config.get("port")
        if self.error_messages.get("ollama"):
            status["ollama"]["error"] = self.error_messages.get("ollama")

        if chroma_config:
            status["chroma"]["host"] = chroma_config.get("host")
            status["chroma"]["port"] = chroma_config.get("port")
        if self.error_messages.get("chroma"):
            status["chroma"]["error"] = self.error_messages.get("chroma")
    ...
    async def _check_service_health(self, service_name: str, base_url: str, health_path: str) -> bool:
        """Check service health (delegates to HealthChecker)."""
        return await self.health_checker.check_service(
