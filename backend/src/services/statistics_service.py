...
class StatisticsService:
    ...
    async def get_daily_email_counts(self, user_id: int, days: int) -> list:
        """Async get daily email statistics (drafts vs sent) for the last N days."""
        try:
            start_date = datetime.now() - timedelta(days=days)

            # Query for drafts created per day
            stmt_drafts = select(
                func.date(LearningPair.created_at).label('date'),
                func.count(LearningPair.id).label('count')
            ).where(
            ...
            result_drafts = await self.db.execute(stmt_drafts)
            drafts_by_date = {str(row.date): row.count for row in result_drafts.all()}

            # Query for emails sent per day (status = 'PAIR_COMPLETED')
            stmt_sent = select(
                func.date(LearningPair.created_at).label('date'),
                func.count(LearningPair.id).label('count')
            ).where(
            ...
            result_sent = await self.db.execute(stmt_sent)
            sent_by_date = {str(row.date): row.count for row in result_sent.all()}

            # Combine and fill missing dates
            chart_data = []
            for i in range(days - 1, -1, -1):
                d = (datetime.now() - timedelta(days=i)).strftime('%Y-%m-%d')
                drafts = drafts_by_date.get(d, 0)
                sent = sent_by_date.get(d, 0)
                chart_data.append({
                    "date": d,
                    "drafts_created": drafts,
                    "emails_sent": sent,
                    "total_activity": drafts + sent
                })
