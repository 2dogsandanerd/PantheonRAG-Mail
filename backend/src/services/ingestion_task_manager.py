...
BM25_INDEX_DIR = Path(__file__).parent.parent / "data" / "bm25_indices"
...
class IngestionTask:
    ...
    )  # List of file assignment objects
    chunk_size: int = 500
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            if self.status
            in [TaskStatus.SUCCESS, TaskStatus.FAILED, TaskStatus.PARTIAL]
            else [],
        }
...
class IngestionTaskManager:
    ...
    def create_task(
        ...
        self.logger.info(
            f"Created task {task_id} for {file_count} files → {collection_name} (user: {user_id or 'anonymous'})"
        )
