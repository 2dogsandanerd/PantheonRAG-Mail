...
class DoclingService:
    ...
    CACHE_DIR = Path("/tmp/docling_cache")  # Simple file-based cache for now
    ...
    async def refine_with_llm(self, markdown_content: str) -> str:
        """
    ...
    async def process_file(self, file_path: str, enable_llm_refinement: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # -- FIAT STRATEGY: Skip Heavy Steps in Lean Mode --
            ...
            if file_path.lower().endswith(".pdf"):
                if is_lean_mode:
                    # In Lean Mode, we assume the PDF is healthy and digital
                    self.logger.info(f"🖖 Fiat Tuning: Überspringe Reparatur/Analyse für {Path(file_path).name}")
                else:
                    # Repair
            ...
            if enable_llm_refinement:
                ...
                markdown_content = await self.refine_with_llm(markdown_content)
