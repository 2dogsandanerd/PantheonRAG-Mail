...
class StatisticsService:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        # Try to get real metrics from Metrics Collector first
        try:
            from src.core.observability.collectors import get_metrics_collector
            collector = get_metrics_collector()
            if collector and hasattr(collector, 'get_latest_metrics'):
                metrics = collector.get_latest_metrics()
                if metrics:
                    # Return real metrics if available
                    return {
                        "rag_query_time": metrics.get("rag_query_time", {"avg": 0, "min": 0, "max": 0}),
                        "llm_response_time": metrics.get("llm_response_time", {"avg": 0, "min": 0, "max": 0}),
                        "email_fetch_time": metrics.get("email_fetch_time", {"avg": 0, "min": 0, "max": 0}),
                        "end_to_end_time": metrics.get("end_to_end_time", {"avg": 0, "min": 0, "max": 0})
                    }
        except Exception:
            pass  # Fallback to empty metrics if collector not available
        
        # Return empty metrics (no data collected yet)
        return {
            "rag_query_time": {"avg": 0, "min": 0, "max": 0},
            "llm_response_time": {"avg": 0, "min": 0, "max": 0},
            "email_fetch_time": {"avg": 0, "min": 0, "max": 0},
            "end_to_end_time": {"avg": 0, "min": 0, "max": 0}
        }

    def get_llm_statistics(self) -> Dict[str, Any]:
        """Get LLM usage statistics."""
        # Try to get real LLM metrics from Metrics Collector first
        try:
            from src.core.observability.collectors import get_metrics_collector
            collector = get_metrics_collector()
            if collector and hasattr(collector, 'get_latest_metrics'):
                metrics = collector.get_latest_metrics()
                if metrics and metrics.get("llm_calls", 0) > 0:
                    return {
                        "total_calls": metrics.get("llm_calls", 0),
                        "total_tokens": metrics.get("llm_tokens", {"input": 0, "output": 0}),
                        "estimated_cost": metrics.get("llm_estimated_cost", 0.0),
                        "model_distribution": metrics.get("llm_model_distribution", {})
                    }
        except Exception:
            pass  # Fallback to empty metrics if collector not available
        
        # Return empty metrics (no data collected yet or token tracking not implemented)
        return {
            "total_calls": 0,
    ...
    async def get_rag_statistics(self) -> Dict[str, Any]:
        ...
        try:
            from src.services.external_rag_connector import get_rag_service, ExternalRAGConnector
            rag_client = await get_rag_service()

            # For ExternalRAGConnector
            if isinstance(rag_client, ExternalRAGConnector):
                try:
                    # In a modular setup, we ask the external service for stats
                    async with rag_client.client.get(f"{rag_client.base_url}/api/v1/stats") as response:
                        if response.status == 200:
                            stats = await response.json()
                            return {
                                "total_queries": stats.get("total_queries", 0),
                                "avg_chunks_used": stats.get("avg_chunks_used", 0),
                                "cache_hit_rate": stats.get("cache_hit_rate", 0.0),
                                "collection_sizes": stats.get("collection_sizes", {}),
                                "total_documents": stats.get("total_documents", 0)
                            }
                except Exception as e:
                    logger.warning(f"Failed to fetch external RAG stats: {e}")
            ...
            # For local RAG (Fallback/Legacy)
            # If ChromaDB is not configured, return empty stats
            client = await rag_client.chroma_manager.get_client_async() if hasattr(rag_client, 'chroma_manager') and rag_client.chroma_manager else None
            if not client:
                return {
                    "total_queries": 0,
                    "avg_chunks_used": 0,
