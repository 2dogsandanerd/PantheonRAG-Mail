...
@router.get("/status")
async def get_services_status(current_user: CurrentUser):
...
async def get_ollama_models(current_user: CurrentUser):
...
async def websocket_status_stream(websocket: WebSocket, token: str = None):
    """WebSocket endpoint for real-time service status updates.
    Authentication via ?token=<access_token> query parameter.
    """
