...
from src.database.models import QueryFeedback
...
async def submit_feedback(
    feedback: FeedbackRequest, current_user: CurrentUser, db=Depends(get_db)
):
    """
    ...
    existing = (
        ...
        .filter(QueryFeedback.query_id == feedback.query_id)
        .first()
    )
    ...
    if existing:
        ...
        return FeedbackResponse(message="Feedback updated", feedback_id=existing.id)
    ...
    return FeedbackResponse(message="Feedback recorded", feedback_id=db_feedback.id)


@router.get("/stats", response_model=FeedbackStats)
async def get_feedback_stats(
    current_user: CurrentUser, days: int = 7, db=Depends(get_db)
):
    """
    ...
    # Total feedback
    total = db.query(QueryFeedback).filter(QueryFeedback.created_at >= since).count()

    # Helpful count
    helpful = (
        db.query(QueryFeedback)
        .filter(QueryFeedback.created_at >= since, QueryFeedback.helpful == True)
        .count()
    )
    ...
    unhelpful = (
        db.query(QueryFeedback)
        .filter(QueryFeedback.created_at >= since, QueryFeedback.helpful == False)
        .count()
    )
    ...
    # Average rating
    avg_rating = (
        db.query(func.avg(QueryFeedback.rating))
        .filter(QueryFeedback.created_at >= since, QueryFeedback.rating.isnot(None))
        .scalar()
        or 0.0
    )
...
async def get_recent_feedback(
    current_user: CurrentUser, limit: int = 10, db=Depends(get_db)
):
